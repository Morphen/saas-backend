    ...
    if (tenant) {
        if (userTemp) {
            ...
            productList.forEach(async (element) => {
                const product = await productServices.findProductInTenant(element.id, domain);
                if (product) {
                    const detalleVenta = await model.DetalleVenta.create({ cantidad: element.cantidad });

                    await venta.setDetalleVenta(detalleVenta);
                    await detalleVenta.setProduct(product);
                }else res.status(400).send("el producto con id "+element.id+" en el tenant "+domain+" no existe")
            });
            res.send(venta)
